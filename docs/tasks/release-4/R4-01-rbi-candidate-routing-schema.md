...
Create schema for identifying RBI candidates based on approved scoping flags and reviewable routing data, not final decision logic.
...
- Implement only the behavior required for R4-01 - RBI Candidate Routing Schema.
...
* Files changed: `packages/shared/src/release-4.ts`, `packages/shared/src/rbac.ts`, `packages/shared/src/index.ts`, `apps/api/src/integrity-foundation/*`
* Implementation summary: Created RBI candidate routing schema types, dataset, Release4IntegrityFoundation class with listRbiCandidates/getRbiCandidate, API controller, and UI navigation.
* Assumptions: No database migrations run; uses static in-memory demo data.
* Tests run: Release 4 shared tests cover candidate listing, tenant isolation, and candidate detail.
