# R5-03 - Reusable Component Contract
...
Define reusable component contracts for consistent table-led and action-led AIM Platform UI implementation.
...
Specify component purpose, props/data contract where knowable, visual behavior, interactions, accessibility notes, use cases, forbidden use, and source basis.
...
Cover AppShell, PageHeader, KPIActionCard, DataTable, FilterRail, drawers, detail tabs, workflow stepper, badges, state components, audit/evidence panels, and export warning panels.

## Data / Mock Data Requirements

Component props must reference domain statuses conservatively and mark unknown props TBD.

## API / Backend Boundary

Component-only planning. Components may be mock/local-state/API-ready depending on later task scope.
...
* Summary: Implemented reusable frontend primitives for page headers, metric/action cards, dense data tables, filter chips, badges, drawer shell, state panels, progress, and boundary banners.
