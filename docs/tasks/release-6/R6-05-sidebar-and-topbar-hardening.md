...
Harden sidebar and topbar to consume registry metadata and show controlled status labels.
...
Use route/navigation registry for nav; add scope/search/digest/action placeholders only.
...
* Summary: Hardened sidebar/topbar with active route state, scope, review, search, digest, quick-action, and boundary labels.
