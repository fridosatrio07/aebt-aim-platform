...
Create dashboard shell where KPI cards drill into filtered action lists instead of static summaries only.
...
- Added dashboard action card service for My Work, pending review, overdue, missing evidence, escalated, and exports pending review.
- Added dashboard-to-action cards on the Release 2 workbench with route hints into filtered queues.
- Counts are derived from scoped action/export data.
...
- Final KPI definitions and dashboard UAT remain open.
