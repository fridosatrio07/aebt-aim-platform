...
Create generic approval workflow primitives for approve, reject, request revision, delegate, and escalate with audit trail.
...
- Added generic approval workflow and workflow transition types plus Prisma models.
- Implemented transitionApproval for approve/reject/request revision/delegate/escalate with RBAC, scope checks, audit events, and draft/preliminary boundary.
- Approve transitions only to approved_for_next_step, never a final domain decision.
...
- Domain-specific approval rules remain future release/SME decisions.
