...
Create generic action item model for pending work across modules with priority, due date, owner, linked object, module, site, and status.
...
- Added shared Release 2 action item types, statuses, priorities, due-state helper, and SPM-01 demo action items.
- Extended Prisma schema with ActionItem foundation model and workflow-oriented enums.
- Action items carry tenant/project/site scope, owner role, required authority, linked object, source basis, and audit-ready status.
...
- Database/shared model task; UI surfaces use the same action records.
