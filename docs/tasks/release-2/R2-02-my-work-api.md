...
Provide tenant-scoped My Work API for user-specific action inbox and filtered work items.
...
- Added Release2WorkflowFoundation.listMyWork with tenant scope, RBAC, owner/role filtering, pagination, filtering, sorting, and audit events.
- Exposed GET /v1/workflow/my-work through the NestJS workflow module.
- Added tests for role-scoped My Work and cross-tenant blocking.
...
- My Work UI consumes the same shared workflow service.
