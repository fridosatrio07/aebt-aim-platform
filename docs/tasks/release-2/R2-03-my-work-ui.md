...
Create My Work / Action Inbox UI that is task-first and links directly to filtered action lists and object detail.
...
- Replaced the Release 1 first screen with a Release 2 task-first workbench.
- Added My Work table with priority, action, owner, due state/date, status, and linked object.
- Kept tenant/project/site selectors, state indicators, and draft/preliminary boundary visible.
...
- GET /v1/workflow/my-work is available for API integration.
