...
Parse Excel/CSV imports into staging with mapping, validation issue capture, duplicate candidate capture, and no direct baseline write.
...
- Added CSV/TSV parser and normalized Excel-row import support into staged import preview.
- Captured missing required fields, missing parent hierarchy, and duplicate candidate issues.
- Added tests for CSV parsing, normalized Excel rows, duplicate candidates, missing parent validation, and baseline write blocking.
...
- Native binary XLSX extraction remains TBD if required by UBT/IT; normalized Excel rows are accepted.
