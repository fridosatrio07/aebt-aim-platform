...
Provide controlled document upload API with metadata validation, object storage integration, RBAC, audit, and tenant isolation.
...
- Added document upload intent API with required metadata validation, tenant scope check, RBAC, audit event, S3-compatible bucket/object-key placement, and draft/preliminary boundary.
- Exposed POST /v1/data-foundation/documents/upload-intent.
- Added tests for upload intent audit and non-final decision boundary.
...
- Real presigned URL/binary storage client wiring remains a UBT/IT environment decision.
