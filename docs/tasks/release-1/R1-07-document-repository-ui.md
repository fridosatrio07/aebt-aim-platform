...
Create document repository list/detail UI with metadata, filters, links, evidence usage, version/status display, and access states.
...
- Added document repository table with metadata, status, version, link count, filter chips, and upload action affordance.
- Exposed GET /v1/data-foundation/documents for repository list retrieval.
- Kept evidence usage as document-link metadata, not evidence-pack export.
...
- Evidence pack builder remains Release 3.
