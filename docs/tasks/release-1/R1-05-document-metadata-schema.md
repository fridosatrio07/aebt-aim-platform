...
Implement document metadata, versioning, linkage, status, confidentiality/access, and source-basis logical foundation.
...
- Extended Prisma schema with Document, DocumentVersion, and DocumentLink foundation models.
- Added shared document metadata/version/link types and demo document records.
- Included status, confidentiality, current version, source basis, and linked object count.
...
- Document metadata appears in API and UI; final permission matrix still needs review.
