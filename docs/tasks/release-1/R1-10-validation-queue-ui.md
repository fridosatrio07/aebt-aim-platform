...
Create validation queue UI for data gaps, duplicate conflicts, invalid units/dates, missing parent/source, and baseline approval routing.
...
- Added validation queue data model, API list, and UI table for open issues.
- Issues show severity, field, row number, assignee role, and blocked baseline status.
- Added tests that queue items block baseline write.
...
- Approval routing is a controlled placeholder; Release 2 owns generic workflow/approval foundation.
